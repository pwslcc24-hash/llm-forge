...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return ACCOUNTS.filter((g) => (q ? String(g.company || "").toLowerCase().includes(q) : true)).filter((g) =>
      customerFilter === "all" ? true : customerFilter === "customer" ? g.isCustomer : !g.isCustomer,
    );
  }, [companySearch, customerFilter]);
      ...
      <div className="mx-auto max-w-[1080px] px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
            Hand-verified list from stored notes, emails, call transcripts, deals, and leads across the Golden DB — grouped by company, with the exact evidence that matched. Launching 8/19. Generated {GENERATED_AT}.
          </p>
        </header>
        ...
        <div className="mt-6 flex items-center gap-1.5 text-[11px] text-slate-400">
          ...
          Matches "cbct", "3d", "second opinion 3d", "so3d" across companies, contacts, notes/calls/emails, deals, and leads. Mass-outreach templates (reengagement emails, FDA-clearance announcement blasts) were identified and excluded — this is per-account signal, not raw keyword hits. Email evidence is subject-line only.
        </div>
      </div>
    </main>
  );
